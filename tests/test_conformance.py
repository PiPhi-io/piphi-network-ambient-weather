...
from piphi_network_ambient_weather.main import app
from piphi_network_ambient_weather import state
...
async def test_runtime_conforms_to_shared_contract_fixtures(monkeypatch) -> None:
    async def fake_refresh(config_id: str):
        return {"connected": True, "config_id": config_id, "command": "refresh"}
    monkeypatch.setattr(state, "refresh_entry", fake_refresh)
